...
class AppointmentTest < ActiveSupport::TestCase
  ...
  test "rescheduling clears reminder delivery tracking" do
    appointment = Appointment.create!(
      client: users(:two), service: services(:one),
      start_time: 5.days.from_now.change(hour: 10, min: 0), status: :confirmado,
      reminder_enqueued_at: Time.current, reminder_sent_at: Time.current
    )

    appointment.update!(start_time: 6.days.from_now.change(hour: 10, min: 0))

    assert_nil appointment.reminder_enqueued_at
    assert_nil appointment.reminder_sent_at
  end
end
